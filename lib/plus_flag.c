...
char *plus_parser_follow(const char *format, va_list *params, int *i, int len)
{
    switch (format[(*i)]) {
        case 'a': return ("");
        case 'A': return ("");
...
char *plus_parser(const char *format, va_list *params, int *i, int len)
{
    char *res = malloc(sizeof(char) * 64);
    res[0] = '+';
    switch (format[(*i) + 1]) {
        case 'd': return (plus_flag_d(va_arg(*params, int), 0));
        case 'i': return (my_strcat(res, plus_flag_d(va_arg(*params, int), 0)));
